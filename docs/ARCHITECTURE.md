...
ADTOF activation order is asserted as MIDI labels `35, 38, 47, 42, 49`, mapped explicitly to kick, snare, tom, hi-hat, and cymbal. Per-class events use the pinned ADTOF peak picker's moving-average subtraction, local-maximum filtering, combination behavior, and thresholds `0.22, 0.24, 0.32, 0.22, 0.30`. Events within 18 ms are cross-class clustered. A cluster may be assigned to only one region, nearest to a slice start inside the −35/+90 ms window. Class selection compares threshold-normalized evidence and keeps secondary classes only when they are within 0.15 of the dominant class. When no peak matches, the region-start window is normalized by class thresholds and may fall back at a 0.70 floor; it stops 20 ms before the next slice boundary to avoid onset bleed.
