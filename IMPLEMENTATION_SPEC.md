...
The committed `dist/DrumSLICE ID.amxd` is a development/reinstallation artifact, not the release-distribution contract. It requires the installed `SliceLabeler` Max package. Root platform installers copy that package and configure the external backend for clone-based use; the AMXD alone remains insufficient. Commercial release requirements and third-party redistribution blockers are tracked in `docs/RELEASE_CHECKLIST.md`, `KNOWN_LIMITATIONS.md`, and `THIRD_PARTY_NOTICES.md`.
